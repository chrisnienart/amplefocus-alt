...
export async function _promptStartTime(app) {
  ...
  const result = await app.prompt("When would you like to start? Choose or enter time (HH:MM):", {
    inputs: [
      {
        label: "Select from suggested",
        type: "select",
      ...
      },
      {
        ...
        type: "text",
        placeholder: "Format: HH:MM (24h)",
        required: false
      }
    ]
  });

  if (result === -1 || result === null) return startTimeOptions[0].value;

  // Handle custom time input
  const customTime = result[1]?.match(/^([01]?[0-9]|2[0-3]):([0-5][0-9])$/);
  if (customTime) {
    const date = new Date();
    date.setHours(parseInt(customTime[1], 10));
    date.setMinutes(parseInt(customTime[2], 10));
    date.setSeconds(0);
    return date;
  }
