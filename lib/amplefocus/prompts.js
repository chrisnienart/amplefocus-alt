...
export async function _promptStartTime(app) {
  ...
  const result = await app.prompt(
      "When would you like to start? /n To enter a time manually, " +
      "choose \"Custom\" in the dropdown and enter the time below in (HH:MM) format.", {
    inputs: [
      {
        label: "Select time", 
        type: "select",
  ...
  if (result[0] === -1 || result[0] === null) return startTimeOptions[1].value;
...
export function _generateCycleOptions(startTime, options, includeCustomOption = false) {
