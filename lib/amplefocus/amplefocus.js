...
export async function _startSession(app, options, dash, startTime, cycles, firstCycle, resume = false, handlePastCycles = false) {
  ...
  workEndTime = new Date(breakEndTime.getTime() - options.breakDuration);
  ...
  for (let currentCycle = firstCycle - 1; currentCycle <= cycles; currentCycle++) {
    ...
    try {
      await _handleWorkPhase(app, warnEndTime, workEndTime, currentCycle);
    }
    catch (error) {
...
export async function _handleWorkPhase(app, warnEndTime, workEndTime, cycleIndex) {
