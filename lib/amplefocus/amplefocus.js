...
export async function _handleWorkPhase(app, options, dash, warnEndTime, workEndTime, cycleIndex) {
...
export async function _logDashboardCycleEndMetrics(app, dash, energy, morale, completion, endTime,options) {
  ...
  tableDict = await _editTopTableCell(tableDict, "End Time", _getISOStringFromDate(endTime));
...
export async function _handleWaitPhase(app, options, startTime) {
  try {
...
export async function _sleepUntil(app, endTime, bell = false, warnTime= 0) {
  // console.log(`Sleeping until ${endTime}...`);
  updateSideBarEmbed(app, endTime);
...
function updateSideBarEmbed(app, endTime) {
