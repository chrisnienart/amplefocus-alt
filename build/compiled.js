(() => {
  ...
  function _calculateDuration(startTime, endTime2) {
    console.debug(`_calculateDuration(${startTime}, ${endTime2})`);
    let start = new Date(startTime);
    let end = new Date(endTime2);
...
`,
  ...
  async function _readDashboard(app, dash) {
...
### ${cycle}`);
  ...
  function _generateStartTimeOptions(customOption = false) {
    const options = [];
  ...
  function parseCustomTime(value) {
    ...
    if (period?.toLowerCase() === "am" && hours === 12)
      hours = 0;
    const customDate = /* @__PURE__ */ new Date();
  ...
  async function _promptStartTime(app) {
    ...
    const result = await app.prompt(
      'When would you like to start? /n To enter a time manually, choose "Custom" in the dropdown and enter the time below in (HH:MM) format.',
      {
        inputs: [
          {
            label: "Select time",
    ...
    if (result[0] === -1 || result[0] === null)
      return startTimeOptions[1].value;
    let startTime;
    if (result[0] === "Custom") {
  ...
  function _generateCycleOptions(startTime, options, includeCustomOption = false) {
    const cycleOptions = [];
    for (let cycles = 1; cycles <= 8; cycles++) {
      const { endTime: endTime2, totalHours, totalMinutes } = _calculateEndTime(options, startTime, cycles);
      const label = `${cycles} cycles (${totalHours} hours ${totalMinutes} minutes, until ${_formatAsTime(endTime2)})`;
  ...
  async function _promptCycleCount(app, options, startTimeValue) {
    ...
    const cycleOptions = _generateCycleOptions(startTime, options, true);
    const result = await app.prompt(
      'Session length - choose cycles from the dropdown or choose "Custom" and enter a number:',
      {
        inputs: [
          {
    ...
    let cycleCount;
    if (result[0] === "Custom") {
      cycleCount = parseInt(result[1]);
    } else {
      cycleCount = parseInt(result[0]);
    }
    console.log(`Session cycle count set to: ${cycleCount}`);
    return cycleCount;
  }
  ...
  function _calculateEndTime(options, startTime, cycles) {
    ...
    const endTime2 = new Date(startTime.getTime() + totalTime);
    const totalMinutes = Math.floor(totalTime / 6e4) % 60;
    const totalHours = Math.floor(totalTime / 36e5);
    return { endTime: endTime2, totalHours, totalMinutes };
  }
...
${content}`);
  ...
  function _getEntriesWithinDates(tableDict, target, startDate, endDate) {
    ...
    let entries = tableDict.filter((row) => {
      let endTime2 = new Date(row["End Time"]);
      console.log(new Date(row["End Time"]));
      return endTime2 >= startDate && endTime2 <= endDate;
    });
    if (target)
  ...
  var plugin = {
    ...
    //===================================================================================
    insertText: {
      "Focus": async function(app, handlePastCycles = false) {
        try {
...
<script>
  ...
  //doesn't work because of app references
  document.getElementById('new-session-button').addEventListener('click', async function() { 
    try {
      console.log("Starting Amplefocus...");
      this.noteUUID = app.context.noteUUID;
      initAmplefocus(app, this.options.amplefocus);
  ...
  function updateParameters(response) {
      ...
      let startNewSessionButton = document.getElementById("new-session-button");
